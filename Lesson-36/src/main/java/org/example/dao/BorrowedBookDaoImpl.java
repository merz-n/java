...
public class BorrowedBookDaoImpl implements BorrowedBookDao{
    ...
    @Override
    public boolean addBorrowedBook(BorrowedBook b) {
        String sql = "INSERT INTO borrowed_books (book_id, reader_id, borrow_date, return_date, status) " +
                "VALUES (?, ?, ?, ?, ?)";

        try (Connection conn = Database.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {

            stmt.setInt(1, b.getBook().getId());
            stmt.setInt(2, b.getReader().getId());
            stmt.setDate(3, java.sql.Date.valueOf(b.getBorrowDate()));

            if (b.getReturnDate() != null) {
                stmt.setDate(4, java.sql.Date.valueOf(b.getReturnDate()));
            } else {
    ...
    public List<BorrowedBook> getBorrowedBooksByReaderId(int readerId) {
        List<BorrowedBook> list = new ArrayList<>();
        String sql = "SELECT bb.id, bb.borrow_date, bb.return_date, bb.status, " +
                "b.id AS book_id, b.title, b.author, b.published_year, b.genre, " +
                "r.id AS reader_id, r.name, r.email, r.phone " +
                "FROM borrowed_books bb " +
                "JOIN books b ON bb.book_id = b.id " +
